    ...
    for i in range(steps - 2):
        if i > 1:
            ...
            for n in range(len(prt)):
                ...
                coords[n].append(2 * coords[n][i - 1] - coords[n][i - 2] + ((tmax - tmin) / steps) ** 2 * (prt[n].f / prt[n].m))
                prt[n].x = coords[n][i]
...
xm = 20
T = 204
vm = math.sqrt(3*1.38*T/part_mas)
for i in range(n):
...
while(True):
    ...
    if n < (len(kin_t)):
        # kin_t[n].sort()
        # area = (kin_t[n][999] - kin_t[n][0])/20
        # a = [0]*20
        # b = []
        # for i in range(20):
        #     b.append(kin_t[n][0] + i*area + area/2)
        #     print(b)
        #     for j in range(1000):
        #         if kin_t[n][j] < kin_t[n][0] + (i+1)*area and kin_t[n][j]  > kin_t[n][0] + i*area:
        #             a[i] = a[i] + 1
        # plt.plot(b, a)
        n, bins, patches = plt.hist(kin_t[n], 25, density=True, facecolor='g', alpha=0.75)
        plt.show()
